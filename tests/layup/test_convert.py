...
def test_convert_round_trip_csv(chunk_size, num_workers):
    ...
    with tempfile.TemporaryDirectory() as temp_out_dir:
        ...
        temp_out_file = os.path.join(temp_out_dir, f"{output_file_stem}.csv")
        convert_cli(
...
def test_convert_one_chunk_one_worker_hdf5(chunk_size, num_workers):
    ...
    with tempfile.TemporaryDirectory() as temp_out_dir:
        output_file_stem = "test_output"
        os.chdir(temp_out_dir)
        temp_out_file = os.path.join(temp_out_dir, f"{output_file_stem}.h5")
        convert_cli(
