...
gauss_soln() : root(), epoch(), csq(), dof(), niter(), x(), y(), z(), vx(), vy(), vz(){
    }
    ...
    gauss_soln(double _root, double _epoch, double _x, double _y, double _z, double _vx, double _vy, double _vz, int _niter, double _dof, double _csq) {
	niter = _niter;
	dof = _dof;
	csq = _csq;
	root = _root;
