...
def matplot_2D(
    ...
    for obj in orb_array:
        ...
        if fade:
            colors = [(14/255,84/255,118/255,a) for a in np.power(np.linspace(0, 1, len(segments)), 5)]
    ...
    if not no_planets:
        ...
        for _, (k, v) in enumerate(planets_dic.items()):
            ...
            axs[1].plot(v[0]*np.cos(theta), v[0]*np.cos(theta)*np.sin(np.radians(v[1])), color=v[2], linestyle='dotted')
...
def matplot_3D(
    ...
    axs.xaxis.set_pane_color((.0, .0, .0, 1.0))  # RGBA: white
    axs.yaxis.set_pane_color((.0, .0, .0, 1.0))
    axs.zaxis.set_pane_color((.0, .0, .0, 1.0))
    ...
    for obj in orb_array:
        ...
        if fade:
            ...
            colors = np.column_stack((np.ones(len(segments)),  # R
                                np.ones(len(segments)),  # G
                                np.ones(len(segments)),  # B
                                alpha))  
    ...
    if not no_planets:
        ...
        planets_dic = dictfilt(planets_dic, planets)
        for _, (k, v) in enumerate(planets_dic.items()):
            axs.plot(v[0]*np.cos(theta), v[0]*np.sin(theta)*np.cos(np.radians(v[1])), v[0]*np.cos(theta)*np.sin(np.radians(v[1])), color=v[2], alpha=0.9, linestyle='dotted')
...
def plotly_3D(
    ...
    for obj in orb_array:
        ...
        fig.add_trace(go.Scatter(
                x=posvec[:,0], y=posvec[:,1], 
                mode="lines",
                line=dict(color=opacity, colorscale='gray', cmin=0, cmax=1,),
                customdata=[obj[0]],
