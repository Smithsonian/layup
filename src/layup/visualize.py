...
from layup.orbit_maths import REQUIRED_COLUMN_NAMES, build_ephem_and_mus, build_planet_lines_cache, conic_lines_from_classical_conic, prepopulate_orbit_variants
...
def build_fig_caches(
    ...
    logger.info(f"Creating cache of all input object and/or planet lines in heliocentric+barycentric && equatorial+ecliptic frames")
...
def visualize_cli(
    ...
    if input_format_infer == "BCART_EQ":
        ...
        if input_plane_infer is None:
            logger.warning("FORMAT=BCART_EQ implies using equatorial plane. Setting --input-plane = equatorial")
    ...
    if input_origin_infer is None:
        ...
        logger.warning(f"--input-origin not provided. Inferring {input_origin_infer} from input file column FORMAT={orbit_format}")
    ...
    if input_plane_infer is None:
        if input_format_infer in ("COM", "BCOM", "KEP", "BKEP"):
            ...
            logger.warning(f"--input-plane not provided. Inferring ecliptic for input file column FORMAT={orbit_format}")
    ...
    if num_orbs > rows.size:
        num_orbs = rows.size
        logger.warning(f"Requested {num_orbs} orbits, but only {rows.size} orbits in input. Capping to {rows.size}")
        # raise Warning("--input-plane is required for CART/BCART formats unless FORMAT column encodes it (e.g. BCART_EQ)")
    if random:
    ...
    if orbit_format_check != orbit_format:
        ...
        raise ValueError(f"FORMAT changed between probe and full read: {orbit_format} -> {orbit_format_check}")
...
def visualize_notebook(
    ...
    if isinstance(data, (str, Path)):
        ...
        if data.dtype.names is None or "FORMAT" not in data.dtype.names:
            logger.error("Structured array input must contain a FORMAT column, which must be one of: ['CART'. 'BCART', 'BCART_EQ', 'KEP', 'BKEP', 'COM', 'BCOM'])")
            raise ValueError("Structured array input must contain a FORMAT column, which must be one of: ['CART'. 'BCART', 'BCART_EQ', 'KEP', 'BKEP', 'COM', 'BCOM'])")
        ...
        if input_origin_infer is None:
            input_origin_infer = "barycentric" if input_format_infer.startswith("B") else "heliocentric"
            logger.warning(f"--input-origin not provided. Inferring {input_origin_infer} from input file format column FORMAT={orbit_format}")
            # raise Warning(f"--input-origin not provided. Inferring {input_origin_infer} from input file column FORMAT={orbit_format}")
        ...
        if input_plane_infer is None:
            if input_format_infer in ("COM", "BCOM", "KEP", "BKEP"):
                input_plane_infer = "ecliptic"
                logger.warning(f"--input-plane not provided. Inferring ecliptic for input file format column FORMAT={orbit_format}")
                # raise Warning(f"--input-plane not provided. Inferring ecliptic for input file column FORMAT={orbit_format}")
            else:
                logger.warning("--input-plane is required for CART/BCART formats unless FORMAT column encodes it (e.g. BCART_EQ)")
                # raise Warning("--input-plane is required for CART/BCART formats unless FORMAT column encodes it (e.g. BCART_EQ)")
