...
# --- set up plots ---
def add_reference_plane_xy(
        fig,
        lines: np.ndarray,
        planet_lines: np.ndarray,
        opacity: float = 0.10
):
    """
...
def plotly_2D(
    ...
    def coords_for(p: PANEL, x: np.ndarray, y: np.ndarray, z: np.ndarray):
        if p == "XY":
            return x, y, "X [AU]", "Y [AU]", True # <-- the final True here is letting later on know to plot this panel equal aspect, as top-down is onto the reference plane
        if p == "XZ":
            return x, z, "X [AU]", "Z [AU]", False # <-- this and YZ are inclination driven so shouldn't be equal aspect
        if p == "YZ":
    ...
    if panels is not None:
        panels_to_show: Tuple[PANEL, ...] = tuple(panels) # < -- if >1 panels, turn whichever options they are into tuple
    ...
    fig = make_subplots(
        rows=1, cols=ncols,
        horizontal_spacing=0.10 if ncols == 2 else 0.02
    )
    ...
    if planet_lines is not None:
        ...
        for i in range(planet_lines.shape[0]):
            ...
            for col, p in enumerate(panels_to_show, start=1):
                xa, ya, _, _, _ = coords_for(p, x, y, z) # <-- grab the correct axes x/y coords for whatever plane it is
                fig.add_trace(
                    go.Scatter(
                        x=xa, y=ya,
                        mode="lines",
    ...
    if orbit_pos is not None:
        ...
        # loop over however many panels we have
        for col, p in enumerate(panels_to_show, start=1):
            xa, ya, _, _, _ = coords_for(p, orbit_pos[:, 0], orbit_pos[:, 1], orbit_pos[:, 2]) # <-- grab the correct axes x/y coords for whatever plane it is
            fig.add_trace(
                go.Scatter(
                    x=xa, y=ya,
                    mode="markers",
    ...
    if plot_sun:
        ...
        # loop over however many panels we have
        for col, p in enumerate(panels_to_show, start=1):
            xa, ya, _, _, _ = coords_for(p, np.array([sx]), np.array([sy]), np.array([sz])) # <-- grab the correct axes x/y coords for whatever plane it is
            fig.add_trace(
                go.Scatter(
                    x=xa, y=ya,
                    mode="markers",
    ...
    # loop over however many panels we have
    for col, p in enumerate(panels_to_show, start=1):
        _, _, xtitle, ytitle, want_equal = coords_for(p, np.array([0.0]), np.array([0.0]), np.array([0.0])) # <-- grab the correct axes x/y titles for whatever plane it is
...
def plotly_3D(
    ...
    fig.update_layout(
        ...
        scene=dict(
            xaxis=dict(title="X [AU]", showbackground=False, gridcolor="rgba(255, 255, 255, 0.02)", zerolinecolor="rgba(255, 255, 255, 0.4)"),
            yaxis=dict(title="Y [AU]", showbackground=False, gridcolor="rgba(255, 255, 255, 0.02)", zerolinecolor="rgba(255, 255, 255, 0.4)"),
            zaxis=dict(title="Z [AU]", showbackground=False, gridcolor="rgba(255, 255, 255, 0.02)", zerolinecolor="rgba(255, 255, 255, 0.4)"),
            aspectmode="data",
            camera=dict(
                center=dict(x=0, y=0, z=-0.25)  # <-- for some reason default camera looks really low to me, negative z shifts scene up in the frame
            )
        )
    )
...
def run_dash_app(
    ...
    app = Dash(__name__, assets_folder="data", external_stylesheets=["https://cdn.jsdelivr.net/npm/katex@0.16.9/dist/katex.min.css"])
    ...
    ORBIT_COLOUR = {
        "night": "rgba(144,167,209,0.70)",
        "day": "rgba(30,60,120,0.85)"
    }
        ...
        for f in figs:
            for tr in getattr(f, "data", []) or []:
                ...
                if isinstance(meta, dict):
                    ...
                    if getattr(tr, "type", None) in ("scatter", "scatter3d") and getattr(tr, "mode", None) == "lines":
                        objids.add(name)
    ...
    inventory_rows = (
        [{"kind": "Planet", "name": p} for p in inv_planets] +
    ...
    def labeled_toggle(left_label: str, toggle_id: str, right_label: str, value: bool, fg: str, width_pix: int=220):
        return html.Div(
            [
                html.Div(left_label, style={**sublabel_style(fg), "marginRight": "14px", "whiteSpace": "nowrap"}, className="toggle-label"),
                daq.ToggleSwitch(
                    id=toggle_id,
                    value=value,
                    size=70,
                ...
                html.Div(right_label, style={**sublabel_style(fg), "marginLeft": "14px", "whiteSpace": "nowrap"}, className="toggle-label")
            ],
            style={
                "display": "flex",
                "alignItems": "center",
    ...
    plane_block_layout = control_block(
        ...
        html.Div(
            [
                html.Div(
                    labeled_toggle("Ecl", "plane-toggle", "Equ", value=plane_equ_default, fg=fg0, width_pix=220),
                    style={"display": "flex", "justifyContent": "center", "width": "100%", "maxWidth": "260px", "margin": "0 auto 14px auto"}
                ),
                html.Div(
                    [
                        html.Div(
                        ...
                        html.Div(
                            "Colour",
                            style={**sublabel_style(fg0), "flex": "0 0 42px", "textAlign": "center"}
                        )
                    ],
                    style={"width": "100%", "maxWidth": "260px", "display": "flex", "gap": "10px", "margin": "0 auto 6px auto"}
                ),
                html.Div(
                    [
                        html.Div(
                        ...
                        html.Button(
                            ...
                            n_clicks=0,
                            style={"flex": "0 0 42px", "width": "42px", "height": "34px", "borderRadius": "10px", "border": f"1px solid {theme0['border']}", "background": theme0["tab_bg"], "color": fg0, "cursor": "pointer", "padding": "0", "display": "flex", "alignItems": "center", "justifyContent": "center"},
                            title="Edit Colour"
                        )
                    ],
                    style={"width": "100%", "maxWidth": "260px", "margin": "0 auto", "display": "flex", "alignItems": "center", "gap": "10px"}
                )
            ],
            style={"display": "flex", "flexDirection": "column", "alignItems": "center", "gap": "10px", "width": "100%", "minWidth": "0"}
        ),
        fg=fg0
    )
    ...
    panel_mode_block = html.Div(
        ...
        style={"width": "220px", "display": "flex", "flexDirection": "column", "alignItems": "center", "marginTop": "14px"},
    )
    ...
    app.layout = html.Div(
        [
                        ...
                        [
                            html.Div(
                                [
                                    ...
                                    html.Div("Controls", style={"fontSize": "22px", "fontWeight": 600, "justifySelf": "center"}),
                                    html.Button(
                                        "x",
                                        id="controls-close",
                                        n_clicks=0,
                            ...
                            html.Div(
                                html.Div(
                                    [
                                        control_block(
                                            "Theme",
                                            labeled_toggle("Day", "theme-toggle", "Night", value=night_default, fg=fg0, width_pix=220),
                                            fg=fg0,
                                            width_px=220
                                        ),
                                        control_block(
                                            "Origin",
                                            labeled_toggle("Helio", "origin-toggle", "Bary", value=origin_bary_default, fg=fg0, width_pix=220),
                                            fg=fg0,
                                            width_px=220
                                        ),
                                        plane_block_layout,
                                        html.Div(
                                            [
                                                control_block(
                                                    "View",
                                                    labeled_toggle("2D", "view-toggle", "3D", value=view_3d_default, fg=fg0, width_pix=200),
                                                    fg=fg0,
                                                    width_px=220
                                                ),
                                                html.Div(
                                                    ...
                                                    style={"display": "none"}  # <-- start hidden since default view is 3D
                                                )
                                            ],
                                            style={"display": "flex", "flexDirection": "column", "alignItems": "center", "gap": "10px", "width": "100%", "minWidth": "0"}
                                        ),
                            ...
                            html.Div(
                                ...
                                children=[
                                    ...
                                    html.Button(
                                        ...
                                        id="objects-close",
                                        n_clicks=0,
                                        style={"border": "none", "background": "transparent", "color": fg0, "fontSize": "22px", "cursor": "pointer"}
                                    )
                                ]
                            ),
                            html.Div(
                                ...
                                children=[
                                    dcc.Input(
                                        ...
                                        value="",
                                        style={"width": "340px", "height": "40px", "borderRadius": "12px", "border": f"1px solid {theme0['border']}", "padding": "0 12px", "outline": "none"}
                                    ),
                                    html.Div(id="obj-count", style={"color": fg0, "opacity": 0.9, "fontSize": "14px"})
                                ]
                            ),
                            html.Div(
                                style={"display": "flex", "gap": "10px", "flexWrap": "wrap", "alignItems": "center"},
            ...
            html.Div(
                ...
                n_clicks=0,
                style={"display": "none", "position": "fixed", "inset": 0, "zIndex": 25000, "background": "rgba(0,0,0,0.55)", "backdropFilter": "blur(6px)", "pointerEvents": "auto"},
                children=[
                    html.Div(
                        id="plane-colour-card",
                        n_clicks=0,
                        style={"width": "min(520px, 92vw)", "margin": "12vh auto", "background": theme0["drawer_bg"], "border": f"1px solid {theme0['border']}", "borderRadius": "16px", "padding": "16px", "boxSizing": "border-box"},
                        children=[
                            html.Div(
    ...
    def toggle_controls(_tab_clicks: int|None, _close_clicks: int|None, is_open: bool):
        trig = ctx.triggered[0]["prop_id"].split(".")[0] if ctx.triggered else "" # <-- this registers what input fires the callback
        if trig == "controls-close":
            return False # <-- if the X button is pressed, force closed
        return not bool(is_open) # <-- if the \/ button is pressed, invert whatever state is_open is in (ie close/open)
    ...
    def style_drawer(is_open: bool, night_mode: bool, drawer_h: float|int|None, drawer_style: dict[str, Any]|None, tab_style: dict[str, Any]|None):
        ...
        ds["zIndex"] = 4000
        ds["pointerEvents"] = "auto" if is_open else "none" # <-- makes sure it wont block the plotly plot elements when closed
        ...
        if is_open and h > 0:
            top_px = max(10, int(h-36-10)) # <-- drawer bottom (h) - button height (36) - small padding (10)
            ts["top"] = f"{top_px}px"
        else:
            ts["top"] = "10px"
    ...
    def toggle_plane_colour_modal(open_clicks: int|None, close_clicks: int|None, night_mode: bool, modal_style: dict[str, Any]|None, card_style: dict[str, Any]|None):
        theme = THEME["night"] if night_mode else THEME["day"]
    ...
    def close_plane_colour_on_backdrop(modal_clicks: int|None, card_clicks: int|None, style: dict[str, Any]|None):
    ...
    def toggle_objects_modal(open: int|None, close: int|None, night_mode: bool, modal_style: dict[str, Any]|None, card_style: dict[str, Any]|None):
        theme = THEME["night"] if night_mode else THEME["day"]
        fg = theme["fg"]
    ...
    @app.callback(
        Output("objects-grid", "quickFilterText"),
        Input("obj-quickfilter", "value")
    )
    ...
    def bulk_visibility(n1: int|None, n2: int|None, n3: int|None, n4: int|None, n5: int|None, n6: int|None, n7: int|None, selected_rows: list[dict[str, Any]]|None, virtual_rows: list[dict[str, Any]]|None, visible_objids: list[str]|None, visible_planets: list[str]|None):
        ...
        def split(rows: list[dict[str, Any]]|None):
            rows = rows or []
            ps = {str(r.get("name")) for r in rows if str(r.get("kind")) == "Planet" and r.get("name") is not None}
            os = {str(r.get("name")) for r in rows if str(r.get("kind")) != "Planet" and r.get("name") is not None}
            return ps, os
    ...
    def capture_view(relayout: dict[str, Any]|None, view_3d: bool, state: dict[str, Any]|None):
        ...
        camera_keys = {"scene.camera", "xaxis.range[0]", "xaxis.range[1]", "yaxis.range[0]", "yaxis.range[1]", "xaxis.autorange", "yaxis.autorange"}
    ...
    def update_orbit_plot(night_mode: bool, view_3d: bool, origin_bary: bool, plane_equ: bool, opacity: float, plane_colour, panel_mode: str, panel_single: str, panel_left: str, panel_right: str, visible_objids: list[str]|None, visible_planets: list[str]|None, view_state: dict[str, object]):
        ...
        for trace in fig.data:
            ...
            if is_planet:
                ...
                trace.line.color = (
                    ...
                    if night_mode else
                    PLANET_COLOURS_DAY.get(name, "rgba(220,220,220,0.7)")
                )
                continue
            ...
            if isinstance(meta, dict) and meta.get("kind") == "epoch":
                trace.marker.color = (
                    "rgba(255,255,255,0.9)" if night_mode
                    else "rgba(30,30,30,0.9)"
                )
        ...
        for trace in fig.data:
            ...
            if getattr(trace, "type", None) in ("scatter", "scatter3d") and getattr(trace, "mode", None) == "lines":
        ...
        if getattr(fig.layout, "scene", None) is not None:
            fig.update_scenes(
                xaxis_gridcolor=grid, xaxis_zerolinecolor=zero,
                yaxis_gridcolor=grid, yaxis_zerolinecolor=zero,
                zaxis_gridcolor=grid, zaxis_zerolinecolor=zero
            )
